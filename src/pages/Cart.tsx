...
const Cart = () => {
  ...
  const [orderSummary, setOrderSummary] = useState<{
    orderId: string;
    customerName: string;
    items: { name: string; price: number; quantity: number; size?: string; colour?: string }[];
    subtotal: number;
    shipping: number;
    total: number;
    phone: string;
    ...
    try {
      ...
      const { error: itemsErr } = await supabase.from('order_items').insert(
        cartItems.map(item => ({
          ...
          colour:     item.selectedColour ?? null,
        }))
      );
...
📦 *Items Ordered*
${cartItems.map(i => {
  const details = [i.selectedSize, i.selectedColour].filter(Boolean).join(', ');
  return `• ${i.name}${details ? ` (${details})` : ''} × ${i.quantity} — EGP ${(i.price * i.quantity).toFixed(2)}`;
}).join('\n')}
      ...
      <div className="py-12 max-w-2xl mx-auto animate-scale-in space-y-8">
        ...
        <div className="border border-pharoic-gold/30 bg-white/[0.02] p-8 space-y-6">
          ...
          <div className="space-y-3 pb-4 border-b border-white/10">
            ...
            {orderSummary.items.map((item, i) => (
              <div key={i} className="flex justify-between text-sm gap-4">
                ...
                <span className="text-white font-medium flex-shrink-0">EGP {(item.price * item.quantity).toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            ...
            {orderSummary.notes && <p className="text-white/40 text-xs">Notes: {orderSummary.notes}</p>}
          </div>
        </div>
        ...
        <Link to="/" className="btn-primary w-full flex items-center justify-center gap-2 text-sm font-bold tracking-widest py-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <div className="lg:col-span-2 border-t border-white/10">
            {cartItems.map((item, i) => (
              <div key={`${item.id}-${item.selectedSize}-${item.selectedColour}`} className="flex gap-8 py-8 border-b border-white/10 group animate-fade-up" style={{ animationDelay: `${i * 80}ms` }}>
                <div className="w-28 aspect-[3/4] overflow-hidden bg-zinc-900 rounded-sm border border-white/5 flex-shrink-0">
                  <img
                    src={(() => { try { const a = JSON.parse(item.image_url); return Array.isArray(a) ? a[0] : item.image_url; } catch { return item.image_url; } })()}
                    alt={item.name}
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      <div className="flex gap-2 flex-wrap pt-1">
                        {item.selectedSize && (
                          <span className="text-[9px] font-bold tracking-widest border border-pharoic-gold/40 text-pharoic-gold px-2 py-0.5 uppercase">
                            {item.selectedSize}
                          </span>
                        )}
                        {item.selectedColour && (
                          <span className="text-[9px] font-bold tracking-widest border border-white/20 text-white/50 px-2 py-0.5 uppercase flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full inline-block border border-white/10" style={{
                              backgroundColor: {
                                Black: '#0A0A0A', White: '#F5F5F5', Beige: '#E8D5B7',
                                Navy: '#001F5B', Olive: '#556B2F', Camel: '#C19A6B',
                                Charcoal: '#36454F', Burgundy: '#800020',
                              }[item.selectedColour] ?? '#888'
                            }} />
                            {item.selectedColour}
                          </span>
                        )}
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center border border-white/10 p-2 gap-4">
                      <button onClick={() => updateQuantity(item.id, -1, item.selectedSize, item.selectedColour)} className="text-white/40 hover:text-white transition-colors"><Minus size={13} /></button>
                      <span className="text-sm font-bold text-white w-4 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, 1, item.selectedSize, item.selectedColour)} className="text-white/40 hover:text-white transition-colors"><Plus size={13} /></button>
                    </div>
                    <button onClick={() => removeFromCart(item.id, item.selectedSize, item.selectedColour)} className="text-white/20 hover:text-red-500 transition-colors flex items-center gap-2 text-xs font-bold tracking-widest uppercase">
          ...
          <div className="space-y-6 animate-slide-right delay-200">
            <div className="bg-white/5 p-8 border border-pharoic-gold/10 space-y-6">
              ...
              <p className="text-white/20 text-[10px] tracking-widest">Shipping: EGP {SHIPPING_COST} flat rate</p>
          ...
          <div className="bg-[#0D0D0D] border border-white/10 w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in">
            ...
            <form onSubmit={handleOrder} className="p-8 space-y-6">
              ...
              <div className="bg-white/5 border border-white/10 p-4 space-y-2">
                ...
                {cartItems.map(item => (
                  <div key={`${item.id}-${item.selectedSize}-${item.selectedColour}`} className="flex justify-between text-sm gap-3">
                    <div>
                      ...
                      {(item.selectedSize || item.selectedColour) && (
                        <p className="text-white/25 text-[10px] tracking-widest">
                          {[item.selectedSize, item.selectedColour].filter(Boolean).join(' · ')}
                        </p>
                      )}
                    </div>
                    <span className="text-white font-bold flex-shrink-0">EGP {(item.price * item.quantity).toFixed(2)}</span>
                  </div>
                ))}
                <div className="h-px bg-white/10 mt-2" />
                <div className="flex justify-between text-sm text-white/40">
                  <span>Subtotal</span><span>EGP {subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm text-white/40">
                  <span>Shipping</span><span>EGP {shipping.toFixed(2)}</span>
                </div>
                <div className="flex justify-between font-bold text-pharoic-gold pt-1">
              ...
              <div>
                ...
                <input type="tel" value={form.phone} onChange={set('phone')} className={inp} placeholder="+20 100 000 0000" required />
              </div>
              ...
              {error && <p className="text-red-400 text-xs border border-red-400/20 bg-red-400/5 p-4">{error}</p>}

              <button type="submit" disabled={loading} className="w-full btn-primary py-5 font-bold tracking-[0.2em] text-sm flex items-center justify-center gap-3 disabled:opacity-60">
                {loading
              ...
              <p className="text-center text-white/20 text-xs">Payment collected upon delivery.</p>
            </form>
          </div>
        </div>
      )}
